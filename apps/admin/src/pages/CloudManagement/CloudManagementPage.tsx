...
import { CloudProvider } from "../../api/regionRequest/types";
...
import { useDeleteRegion } from "../../api/regionRequest/deleteRegion";
...
const CloudManagementPage = () => {
  const { Modal: RegionModal, open: openRegionModal, isOpen: isOpenRegionModal, close: closeRegionModal } = useModal()
  const awsRegions = useGetRegionByFilter({ cloudProvider: CloudProvider.AWS })
  const azureRegions = useGetRegionByFilter({ cloudProvider: CloudProvider.AZURE })
  ...
  const deleteRegionFromServer = async () => {
    ...
    if(selectedRegion.cloudProvider===CloudProvider.AWS){
      awsRegions.refetch()
    } else {
  ...
  return (
    <Container>
      <Grid container spacing={3}>
        ...
        <Grid item xs={12}>
          <Stack direction={'row'} width={'100%'} alignItems={'center'} justifyContent={'space-between'} spacing={2} mt={4}>
            <Typography sx={{ color: 'text.primary' }} variant="h6">Regions</Typography>
          ...
          <Grid item xs={12}>
            <Stack direction={'row'} spacing={2}>
              <Stack width='100%' spacing={1}>
                <Typography sx={{ color: 'text.primary' }} variant="body1">AWS</Typography>
                {
                  awsRegions.isLoading ?
                    <ShowSkeleton viewType="table" /> :
                    <Table data={awsRegions.data || []} columnsProp={AwsRegionTableColumn} />
                }
              </Stack>
              <Stack width='100%' spacing={1}>
                <Typography sx={{ color: 'text.primary' }} variant="body1">AZURE</Typography>
                {
                  azureRegions.isLoading ?
                    <ShowSkeleton viewType="table" /> :
                    <Table data={azureRegions.data || []} columnsProp={AzureRegionTableColumn} />
                }
              </Stack>
            </Stack>
          </Grid>
      ...
      {
        isOpenRegionModal &&
        <RegionModal title={"Region Form"} showCloseIcon open={isOpenRegionModal}>
          <RegionForm onActionPerformed={onActionPerformed} selectedRegion={selectedRegion} />
        </RegionModal>
      }

      {
        ...
        <DeleteDialog
          ...
          icon={<Delete />}
          action={
            <Button variant="contained" onClick={deleteRegionFromServer} disabled={deleteRegion.isPending} color="error">{deleteRegion.isPending ? 'Deleting':'Delete'}</Button>
          }
        />
      }
    </Container>
