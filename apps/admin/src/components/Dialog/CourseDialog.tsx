...
const defaultValues: CourseFormValues = {
    ...
    price: 0,
    test: false,
    content: [{ title: "", time: new Date().toISOString().slice(0, 16), description: "" }],
    certificate: false,
    labs: [],
    points: 0,
};
...
const CourseDialog = () => {
    ...
    const onNext = async () => {
        ...
        if (activeStep === 0) fieldsToValidate = ["name", "description", "price", "test"];
        ...
        <>
            ...
            <Modal title="Create Course" showCloseIcon open={isOpen}>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    {/* Step 2: Content */}
                    {activeStep === 1 && (
                        <CourseContent control={control} />
                    )}

                    {/* Step 3: Configuration */}
                    {activeStep === 2 && (
                        <CourseConfiguration control={control} />
                    )}

                    {/* Step 4: Confirmation */}
                    {activeStep === 3 && (
                       <CourseConfirmation getValues={getValues} />
                    )}
