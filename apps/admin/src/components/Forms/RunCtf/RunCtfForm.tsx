...
import { AzureRegion, CloudProvider } from "../Region/types";
...
const schema = yup.object().shape({
    ...
    maxParticipants: yup.number().required("Participants count is required").typeError("Participants count must be a number"),
...
const RunCtfForm: React.FC<RunCtfFormProps> = ({ close }) => {
    ...
    const azureRegionRes = useGetRegionByFilter({cloudProvider: CloudProvider.AZURE, })
    const azureRegions:SelectOption[] = azureRegionRes.data?.map(item => ({label: item.AZURERegion as string, value: item.AZURERegion as string})) || [];


    const ctfSelectOPtions:SelectOption[] = ctf.data?.map((ctf) => ({
        value: ctf.id,
        label: ctf.name,
    })) || [];
            ...
            <Controller
                ...
                render={({ field, fieldState: { error } }) => (
                    <FormControl fullWidth margin="normal" error={!!error}>
                        ...
                        <Select {...field} label={azureRegionRes.isLoading ? "Loading regions..." : "Region"}>
