import { Divider, Stack, Button, Box, useTheme, InputAdornment, Typography, TextField, Chip, Autocomplete, Checkbox, FormControlLabel, MenuItem } from '@mui/material';
...
const schema = yup.object({
    ...
    type: yup.string().required('Lab type is required').oneOf(Object.values(LabType), 'Invalid lab type'),
    price: yup.number().required('Price is required').typeError('Must be a number'),
    point: yup.number().required('Point is required').typeError('Must be a number'),
    difficult: yup.string().required('Lab difficulty is required').oneOf(Object.values(Difficulty), 'Invalid difficulty')
});
                    ...
                    value={editMode ? (
                        ...
                        <Stack direction="row" spacing={1} flexWrap="wrap" mt={1}>
                            {data.tag.map((t: string, i: number) => (
                                <Chip key={i} label={t.replace(/[\[\]"]/g, "")} variant="outlined" />
                            ))}
                        </Stack>
                    )
                    }
