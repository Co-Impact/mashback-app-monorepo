import { Divider, Stack, Button, Box, Typography, TextField, IconButton, MenuItem, } from '@mui/material';
...
import { Difficulty, ILab } from '../../api/types';
...
const ManageLabSteps: FC<ManageLabBasicDetailsProps> = ({ data }) => {
    ...
    const {
        ...
        defaultValues: {
            labsSteps: data?.labsSteps.map(item => ({ title: item.title, description: item.description, step: item.step, id: item.id })) || [],
        },
    });
                    ...
                    value={editMode ? (
                        ...
                        <Stack spacing={1}>
                            <Stack spacing={2} mt={2}>
                                ...
                                {
                                    fields.map((item, idx) => (
                                        counter === idx && <Box key={item.id} p={2} border={1} borderRadius={2} sx={{ marginTop: "12px" }}>
                                            <Stack direction={'row'} justifyContent={'space-between'} alignItems={'center'}>
                        ...
                        <Stack width={'100%'}>
                            <Table data={data?.labsSteps || []} columnsProp={labStepsColumn} />
                        </Stack>
                    )}
                />
